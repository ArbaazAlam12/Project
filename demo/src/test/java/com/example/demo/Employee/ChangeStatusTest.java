...
public class ChangeStatusTest {
    ...
    public void Change_Status_To_Active()throws Exception{
        when(employeeService.changeStatusActive(Employee1.getId())).thenReturn(Employee2);
        mockMvc.perform(MockMvcRequestBuilders
                        .post("/employee/changeStatus/1")
    ...
    public void Change_Status_To_Active_Error()throws Exception{
        when(employeeService.changeStatusActive(1l)).thenThrow(new RuntimeException("Unable to change status"));
        mockMvc.perform(MockMvcRequestBuilders
                        .post("/employee/changeStatus/1")
                        .contentType(MediaType.APPLICATION_JSON)
                )
